...
export function AudioAttachmentPlayer({
  ...
  const [playbackState, setPlaybackState] = useState<PlaybackState>(() => createInitialPlaybackState(src))

  const currentState =
    playbackState.srcKey === src ? playbackState : createInitialPlaybackState(src)

  const patchState = useCallback(
    (patch: Partial<Omit<PlaybackState, 'srcKey'>>) => {
      setPlaybackState((prev) => {
        const base = prev.srcKey === src ? prev : createInitialPlaybackState(src)
  ...
  return (
    ...
    >
      <audio
        ...
        onError={() => patchState({ hasError: true, isPlaying: false, isReady: false })}
      />
      ...
      <div className={styles.controlsRow}>
        <input
          ...
          step={0.1}
          value={progressValue}
          disabled={!currentState.isReady || currentState.hasError || progressMax <= 0}
          onChange={(event) => {
