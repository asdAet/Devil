...
type MockCropperProps = {
  ...
  onCropComplete?: (croppedArea: MockCropArea, croppedAreaPixels: MockCropArea) => void
  onCropAreaChange?: (croppedArea: MockCropArea, croppedAreaPixels: MockCropArea) => void
}
...
describe('AvatarCropModal', () => {
  ...
  it('normalizes crop from completed pixel area and media natural size', () => {
    ...
    render(<AvatarCropModal open image="blob:test-image" onCancel={vi.fn()} onApply={onApply} />)

    expect(cropperState.props?.style?.cropAreaStyle).toEqual({ width: '100%', height: '100%' })
  ...
  it('does not dismiss on backdrop click or Escape and only triggers actions via buttons', () => {
    ...
    render(<AvatarCropModal open image="blob:test-image" onCancel={onCancel} onApply={onApply} />)
