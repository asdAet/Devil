...
const IconHome = () => (
  <svg className={styles.drawerIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <aside className={styles.sidebar}>
      {drawerOpen && (
        <div className={styles.drawerOverlay} onClick={() => setDrawerOpen(false)}>
          ...
          >
            ...
            <button
              type="button"
              className={[styles.drawerItem, isActive('/') ? styles.drawerActive : ''].filter(Boolean).join(' ')}
              onClick={() => navAndClose('/')}
            >
              <IconHome />
              <span>Главная</span>
            ...
            {user && (
              <button
                type="button"
                className={[styles.drawerItem, isActive('/friends') ? styles.drawerActive : ''].filter(Boolean).join(' ')}
            ...
            {user && (
              <button
                type="button"
                className={[styles.drawerItem, isActive('/groups') ? styles.drawerActive : ''].filter(Boolean).join(' ')}
            ...
            <button
              type="button"
              className={[styles.drawerItem, isActive('/settings') ? styles.drawerActive : ''].filter(Boolean).join(' ')}
            ...
            {user && (
              <button
                type="button"
                className={[styles.drawerItem, styles.drawerDanger].filter(Boolean).join(' ')}
                onClick={() => {
                  setDrawerOpen(false)
                  onLogout()
                }}
        ...
        <div className={styles.header}>
          ...
          <div className={styles.searchBox}>
            ...
            <input
              ...
              placeholder="Поиск по пользователям, группам и сообщениям..."
              aria-label="Поиск"
              value={searchQuery}
      ...
      {user ? (
        ...
        <div className={styles.authButtons}>
          ...
          <Button variant="ghost" fullWidth onClick={() => onNavigate('/register')}>
            Регистрация
          </Button>
        </div>
      )}
