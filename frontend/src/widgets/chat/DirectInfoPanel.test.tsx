...
describe('DirectInfoPanel', () => {
  it('renders AudioAttachmentPlayer in attachments tab for audio items', async () => {
    ...
    await waitFor(() => {
      ...
      expect(chatControllerMock.getRoomAttachments).toHaveBeenCalledWith('dm_1', { limit: 60 })
    })

    const tabButtons = screen.getAllByRole('button')
    fireEvent.click(tabButtons[1])

    expect(await screen.findByTestId('audio-attachment-player')).toBeInTheDocument()
