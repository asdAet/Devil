...
export function MessageInput({
  ...
  return (
    <div className={[styles.wrapper, rateLimitActive ? styles.blocked : ''].filter(Boolean).join(' ')}>
      {rateLimitActive && (
        <div className={styles.rateLimitBanner} role="status" aria-live="polite">
          Слишком много сообщений. Подождите <span className={styles.rateLimitTimer}>{rateLimitSeconds} сек</span>
        </div>
      )}
      ...
      {uploading && (
        <div className={styles.uploadBar}>
          ...
          {onCancelUpload && (
            <button type="button" className={styles.uploadCancelBtn} onClick={onCancelUpload}>
              Отменить загрузку
            </button>
          )}
        </div>
      ...
      {replyTo && (
        <div className={styles.replyBar}>
          ...
          <button type="button" className={styles.replyBarClose} onClick={onCancelReply} aria-label="Отменить ответ">
            <IconClose />
          </button>
        </div>
      )}
      ...
      {hasQueuedFiles && (
        <div className={styles.previewWrap}>
          <div className={styles.previewHeader}>
            <span className={styles.previewTitle}>Вложения: {pendingFiles.length}</span>
            {onClearPendingFiles && !uploading && (
              <button type="button" className={styles.previewClearBtn} onClick={onClearPendingFiles}>
                Очистить
              </button>
            )}
          </div>
            ...
            {previewItems.map((item, index) => (
              <div key={`${item.file.name}-${index}`} className={styles.previewItem}>
                {item.url && item.file.type.startsWith('image/') && (
                  <img className={styles.previewThumb} src={item.url} alt={item.file.name} />
                )}
                {item.url && item.file.type.startsWith('video/') && (
                  <video className={styles.previewThumb} src={item.url} controls preload="metadata" playsInline />
                )}
                {!item.url && (
                  <div className={styles.previewFileBadge}>{item.file.name.slice(0, 2).toUpperCase()}</div>
                )}
