...
import styles from "../../styles/chat/CustomEmojiPicker.module.css";
import { useVirtualEmojiGrid } from "./lib/useVirtualEmojiGrid";
import { useVirtualHorizontalList } from "./lib/useVirtualHorizontalList";

export type CustomEmojiPickerProps = {
...
const resolveInitialPackId = (
  packs: readonly CustomEmojiPackSummary[],
): string => packs[0]?.id ?? "";
...
const PACK_BUTTON_WIDTH = 42;
const PACK_BUTTON_GAP = 6;

const scrollPackIntoView = (node: HTMLDivElement, index: number): void => {
  const itemLeft = index * (PACK_BUTTON_WIDTH + PACK_BUTTON_GAP);
  const itemRight = itemLeft + PACK_BUTTON_WIDTH;
  ...
  const nextScrollLeft =
    itemLeft < visibleLeft
      ? itemLeft
      : itemRight - node.clientWidth + PACK_BUTTON_GAP;
  node.scrollTo({
...
export function CustomEmojiPicker({
  ...
  const filteredEmojis = useMemo(() => {
    const emojis = activePack?.emojis ?? [];
  ...
  }, [activePack?.emojis, deferredSearchQuery]);
  const virtualGrid = useVirtualEmojiGrid({
    itemCount: filteredEmojis.length,
    scrollRoot: gridViewportRoot,
  });
  const virtualPackFooter = useVirtualHorizontalList({
    gap: PACK_BUTTON_GAP,
    itemCount: packSummaries.length,
    itemWidth: PACK_BUTTON_WIDTH,
    scrollRoot: footerViewportRoot,
  });
  ...
  }, [activePackId, gridViewportRoot, deferredSearchQuery]);
  ...
  useEffect(() => {
    ...
    const activePackIndex = packSummaries.findIndex(
      (pack) => pack.id === activePackIdValue,
    );
    if (activePackIndex >= 0) {
      scrollPackIntoView(footerViewportRoot, activePackIndex);
    }
  }, [activePackIdValue, footerViewportRoot, packSummaries]);
  ...
  return (
    ...
    >
      ...
      >
        <div className={styles.header}>
          <div className={styles.headerCopy}>
            ...
            <span className={styles.headerMeta}>
              {filteredEmojis.length} из {activePackSummary?.emojiCount ?? 0}
            </span>
          </div>
          <input
            type="search"
            ...
            placeholder="Поиск"
            aria-label="Поиск эмоджи в активном паке"
            autoComplete="off"
            spellCheck={false}
          />
          <button
        ...
        >
          <div
            className={styles.footerTrack}
            style={{ width: virtualPackFooter.totalWidth }}
          >
            {virtualPackFooter.items.map(({ index, style }) => {
