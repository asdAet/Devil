...
import { useConversationList, type FilterTab } from '../../shared/conversationList/ConversationListProvider'
...
export function ConversationList({ onNavigate }: Props) {
  ...
  const getItemPath = useCallback((item: { type: string; slug: string; name: string }) => {
    if (item.type === 'direct') {
      return `/direct/@${encodeURIComponent(item.name)}`
    }
    ...
    <>
      <div className={styles.tabs}>
        {TAB_LABELS.map(({ key, label }) => (
          <button
            key={key}
            type="button"
            className={[styles.tab, filter === key ? styles.active : ''].filter(Boolean).join(' ')}
      ...
      {isGlobalMode ? (
            ...
            <section className={styles.globalSection}>
              ...
              {globalResults.groups.map((group) => (
                <button
                  ...
                  type="button"
                  className={styles.globalItem}
                  onClick={() => onNavigate(`/rooms/${encodeURIComponent(group.slug)}`)}
                >
                  <Avatar username={group.name} size="tiny" />
                  <div className={styles.globalMeta}>
                    <span className={styles.globalPrimary}>{group.name}</span>
                    {group.username && <span className={styles.globalSecondary}>@{group.username}</span>}
                  </div>
                </button>
              ))}
            </section>
            ...
            <section className={styles.globalSection}>
              ...
              {globalResults.messages.map((message) => (
                <button
                  ...
                  type="button"
                  className={styles.globalItem}
                  onClick={() => onNavigate(`/rooms/${encodeURIComponent(message.roomSlug)}?message=${message.id}`)}
                >
                  <div className={styles.globalMeta}>
                    <span className={styles.globalPrimary}>{message.username} • {message.roomName || message.roomSlug}</span>
                    <span className={styles.globalSecondary}>{message.content}</span>
                  </div>
                </button>
              ))}
            </section>
        ...
        <EmptyState
          ...
          description={searchQuery ? 'Попробуйте другой запрос' : 'Начните новый диалог'}
        />
      ) : (
