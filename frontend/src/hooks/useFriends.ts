...
export function useFriends(): UseFriendsResult {
  ...
  const removeFriendCb = useCallback(async (userId: number) => {
    await friendsController.removeFriend(userId)
    void reload()
  }, [reload])

  const blockUserCb = useCallback(async (username: string) => {
    await friendsController.blockUser(username)
    void reload()
  }, [reload])

  const unblockUserCb = useCallback(async (userId: number) => {
