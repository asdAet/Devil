...
import type { Group, GroupMember, PinnedMessage } from '../entities/group/types'
...
export function useGroupDetails(slug: string): UseGroupDetailsResult {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const msg = err && typeof err === 'object' && 'message' in err ? (err as { message: string }).message : 'Ошибка загрузки'
      setError(msg)
    } finally {
      setLoading(false)
    }
  ...
  const updateGroupCb = useCallback(async (data: UpdateGroupInput) => {
  ...
  const kickMemberCb = useCallback(async (userId: number) => {
    await groupController.kickMember(slug, userId)
    void reload()
  }, [slug, reload])

  const banMemberCb = useCallback(async (userId: number, reason?: string) => {
    await groupController.banMember(slug, userId, reason)
    void reload()
  }, [slug, reload])

  const muteMemberCb = useCallback(async (userId: number, durationSeconds?: number) => {
    await groupController.muteMember(slug, userId, durationSeconds)
    void reload()
  }, [slug, reload])

  const unmuteMemberCb = useCallback(async (userId: number) => {
    await groupController.unmuteMember(slug, userId)
    void reload()
  }, [slug, reload])

  const pinMessageCb = useCallback(async (messageId: number) => {
    await groupController.pinMessage(slug, messageId)
    const updated = await groupController.getPinnedMessages(slug)
    setPins(updated)
  }, [slug])

  const unpinMessageCb = useCallback(async (messageId: number) => {
