...
const friendListSchema = z.object({ items: z.array(friendshipSchema) }).passthrough()
const requestListSchema = z.object({ items: z.array(friendshipSchema) }).passthrough()
...
const mapIncomingRequest = (dto: z.infer<typeof friendshipSchema>): FriendRequest => ({
  id: dto.id,
  userId: dto.user.id,
  username: dto.user.username,
  profileImage: dto.user.profileImage ?? null,
...
const mapOutgoingRequest = (dto: z.infer<typeof friendshipSchema>): FriendRequest => ({
  id: dto.id,
  userId: dto.user.id,
  username: dto.user.username,
  profileImage: dto.user.profileImage ?? null,
...
export const decodeSendFriendRequestResponse = (input: unknown): SendFriendRequestResponse => {
  const parsed = decodeOrThrow(sendRequestResponseSchema, input, 'friends/send-request')
...
const blockedListSchema = z.object({ items: z.array(friendshipSchema) }).passthrough()

const mapBlockedUser = (dto: z.infer<typeof friendshipSchema>): BlockedUser => ({
  id: dto.id,
  userId: dto.user.id,
  username: dto.user.username,
  profileImage: dto.user.profileImage ?? null,
