...
class GroupController {
  public async createGroup(data: { name: string; description?: string; isPublic?: boolean; username?: string | null }): Promise<Group> {
    const group = await apiService.createGroup(data)
    emitConversationListRefresh()
    return group
  }

  public async getPublicGroups(params?: { search?: string; page?: number; pageSize?: number }): Promise<{ items: GroupListItem[]; total: number }> {
    return apiService.getPublicGroups(params)
  }

  public async getMyGroups(params?: { search?: string; page?: number; pageSize?: number }): Promise<{ items: GroupListItem[]; total: number }> {
  ...
  public async updateGroup(slug: string, data: UpdateGroupInput): Promise<Group> {
  ...
  public async getGroupMembers(slug: string, params?: { page?: number; pageSize?: number }): Promise<{ items: GroupMember[]; total: number }> {
  ...
  public async banMember(slug: string, userId: number, reason?: string): Promise<void> {
  ...
  public async muteMember(slug: string, userId: number, durationSeconds = 3600): Promise<void> {
  ...
  public async getBannedMembers(slug: string): Promise<{ items: BannedMember[]; total: number }> {
    return apiService.getBannedMembers(slug)
  }

  public async createInvite(slug: string, data?: { maxUses?: number; expiresInHours?: number }): Promise<GroupInvite> {
  ...
  public async approveJoinRequest(slug: string, requestId: number): Promise<void> {
    return apiService.approveJoinRequest(slug, requestId)
  }

  public async rejectJoinRequest(slug: string, requestId: number): Promise<void> {
